...
export const ItemSpecifics = ({ productDetails, setProductDetails }) => {
  // Lấy specifics từ productDetails
  const specifics = productDetails?.specifics || {};
  // State để chuyển đổi giữa chế độ edit và không edit
  const [isEditing, setIsEditing] = useState(false);

  // Hàm để xử lý khi thêm mới một specific
  const handleAddSpecific = () => {
    if (Object.keys(specifics).length < 15) {
      const newKey = `New Attribute ${Object.keys(specifics).length + 1}`;
      setProductDetails({
        ...productDetails,
        specifics: {
          ...specifics,
  ...
  // Hàm để xử lý khi xóa một specific
  ...
  // Hàm để cập nhật giá trị key hoặc value của một specific
          ...
          <table className="table-fixed w-full">
            <tbody>
              {Object.keys(specifics).length > 0 ? (
                ...
                <tr>
                  <td className="px-4 py-2" colSpan="3">No specifics available</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <table className="table-fixed w-full">
            <tbody>
              {Object.keys(specifics).length > 0 ? (
                ...
                <tr>
                  <td className="px-4 py-2" colSpan="2">No specifics available</td>
                </tr>
              )}
            </tbody>
          </table>
