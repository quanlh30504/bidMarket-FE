...
export const ProductsDetailsPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isAutoOn) {
      try {
        const response = await axios.post('/api/placeBid', { bidAmount });
        console.log('Bid placed successfully:', response.data);
      } catch (error) {
        console.error('Error placing bid:', error);
      }
    } else {
      // Handle auto bid logic here
    }
  };
  return (
    <>
          ...
          <div className="flex justify-between gap-8">
            ...
            <div className="w-1/2">
              ...
              <div className="p-5 px-10 shadow-s3 py-8">
                <form className="flex gap-3 justify-between" onSubmit={hand}>

                  {isAutoOn ? (
                    <>
                    <input className={commonClassNameOfInput} type="number" name="maxBid" placeholder="Max Bid" />
                    <button type="button" className="bg-gray-100 rounded-md px-5 py-3">
                    <AiOutlinePlus />
                  </button>
                    <input className={commonClassNameOfInput} type="number" name="increAmount" placeholder="Increment Amount" />
                    <button type="button" className="bg-gray-100 rounded-md px-5 py-3">
                    <AiOutlinePlus />
                  </button>
                  ...
                  : (
                    <>
                    <input className={commonClassNameOfInput} type="number" name="bidAmount" placeholder="Bid Amount" />
                    <button type="button" className="bg-gray-100 rounded-md px-5 py-3">
