...
export const Watchlist = () => {
  ...
  const fetchWatchlist = async (page) => {
    try {
      ...
      // console.log(response.data.content);{userBids[item.auctionId]
                  ? `$${userBids[item.auctionId]?.amount}` // Hiển thị giá bid
                  : "Loading..."}
  ...
  const handleRemoveFromWatchlist = async (watchlistId) => {
    try {
      const response = await axiosClient.delete(
        `/api/watchlist/remove/${watchlistId}`
      );
      ...
      for (const item of items) {
        try {
          const response = await axiosClient.get(
            `/api/bids/auction/${item.auctionId}`
          );
          console.log(response.data);
          bidsData[item.auctionId] = response.data;
        } catch (error) {
          console.error(
            `Error fetching bid for auctionId ${item.auctionId}`,
            error
          );
          bidsData[item.auctionId] = null; // Nếu lỗi, đặt null
        }
      }
        ...
        <tbody>
          {items.map((item, index) => (
            <tr key={index} className="bg-white border-b hover:bg-gray-50">
              ...
              <td className="px-6 py-4 text-center">{userBids[item.auctionId]
                  ? `$${userBids[item.auctionId]?.bidAmount}` // Hiển thị giá bid
                  : "Loading..."}</td>
              <td className="px-6 py-4 text-center">
                <div
                  className={`inline-block w-24 px-2 py-1 text-sm border-2 rounded-full text-center truncate rounded-full ${
                    item.status === "Winning"
                      ? "text-green border-emerald-500"
              ...
              <td className="px-6 py-4 text-center text-red-500">
                {" "}
                {timeLeftData[index] || "Calculating..."}
              </td>
