  ...
  const fetchWatchlist = async (page, searchTerm) => {
    try {
      const response = await axiosClient.get(`/api/watchlist/user/${userId}`, {
        params: {
          page: page - 1,  // Backend sử dụng index bắt đầu từ 0
          size: itemsPerPage,
          sortBy: 'createdAt',  // Hoặc bạn có thể cho phép sắp xếp theo tham số khác
          sortDirection: 'DESC',
          // Nếu cần, có thể thêm tham số tìm kiếm vào đây, ví dụ như searchTerm
        }
      });
