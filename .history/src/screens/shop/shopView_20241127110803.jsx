...
export const ShopView = () => {
  ...
  //     try {
  //       const response = await axios.get(`http://localhost:8080/api/users/${sellerId}`);
    ...
    const fetchSellerData = async () => {
      try {
        ...
        if (response.data.content.length > 0) {
          ...
          setSellerData({
            name: `Seller ${firstProduct.sellerId}`, // Hoặc bất kỳ thông tin nào từ sản phẩm
            location: 'Ha Noi, Vietnam', // Bạn có thể cập nhật từ API nếu cần
          });
        }
      } catch (error) {
        console.error('Error fetching seller data:', error);
