...
export const ShopView = () => {
  ...
  useEffect(() => {
    const fetchSellerData = async () => {
      try {
        ...
        // console.log(productsResponse.data.content);
      } catch (error) {
  ...
  // Kiểm tra các giá trị trước khi ánh xạ để tránh lỗi
  const productDto = item.productDto || {};
  const productImages = productDto.productImages || [];
  const mappedItem = {
    id: productDto.id || "", // Đảm bảo có ID, nếu không để chuỗi rỗng
    productImages:
      productImages.find((img) => img.isPrimary)?.imageUrl ||
      "https://via.placeholder.com/150", // Hình ảnh mặc định
    name: productDto.name || "Unknown Product", // Tên sản phẩm mặc định nếu thiếu
    description: productDto.description || "No description available", // Mô tả mặc định
    productStatus: productDto.productStatus || "N/A", // Trạng thái mặc định
    sellerId: productDto.sellerId || "Unknown Seller", // Seller mặc định
    stockQuantity: productDto.stockQuantity || 0, // Số lượng mặc định
    categories: productDto.categories || "Uncategorized", // Category mặc định
    bprice: item.startingPrice || 0, // Giá khởi điểm mặc định
    price: item.currentPrice || 0, // Giá hiện tại mặc định
    bids: item.bidCount || 0, // Số lượt đặt giá mặc định
    timeleft: calculateTimeLeft(item.endTime) || "N/A", // Thời gian còn lại mặc định
  };
