  ...
  return (
    ...
    <section className="product-home">
      ...
      {sellerProducts.map((item) => {
        ...
        <Title className="pt-8" level={4}>All Auction</Title>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 my-8">
          {paginatedProducts.map(sellerProducts => (
            <div key={sellerProducts.id} className="border p-4 rounded-lg shadow-md">
              <img src={sellerProducts.productImages} alt={sellerProducts.name} className="w-full h-48 object-cover rounded-lg" />
              <Title level={4} className="mt-4">{sellerProducts.name}</Title>
              <Caption>{sellerProducts.description}</Caption>
              <div className="flex justify-between items-center mt-2">
                <span className="text-green-500">${sellerProducts.price}</span>
                <span className="text-gray-500">{sellerProducts.timeleft}</span>
              </div>
            </div>
          ))}
        </div>
