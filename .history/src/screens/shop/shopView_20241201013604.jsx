...
export const ShopView = () => {
  ...
  useEffect(() => {
    const fetchSellerData = async () => {
      try {
        const sellerResponse = await axiosClient.get(
          `/api/users/${sellerId}/accountInfo`
        );
        ...
        const isFollowingResponse = await axiosClient.get(
          `/api/follows/${sellerId}/isFollowing`,
          {
        ...
        const followersCountResponse = await axiosClient.get(
          `/api/follows/${sellerId}/followersCount`
        );
      ...
      <div className="pt-16 relative">
        <div className="bg-primary pt-8 h-56 relative content overflow-hidden">
          <img
            ...
            src="https://bidout-wp.b-cdn.net/wp-content/uploads/2022/10/client-right.png"
          ></img>
          <img
          ...
          <div className="flex items-center gap-3 w-[75%] m-auto">
            ...
            <div className="flex items-center gap-8 h-32">
              ...
              <div className="w-96 flex justitfy-center ltr">
                {isFollowing ? (
                  <button
                    className=" ms-8 w-24 px-2 py-1 text-sm border-2 rounded-full text-white border-green bg-green"
                ...
                <button className=" ms-8 w-24 px-2 py-1 text-sm border-2 rounded-full text-white border-green bg-green flex items-center gap-1">
                  <IoChatbubbleEllipsesOutline className="ml-3" />
                  Chat
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8 my-8">
              {sellerProducts.map((item) => {
                const mappedItem = {
                  ...
                  productImages:
                    item.productDto.productImages.find((img) => img.isPrimary)
                      ?.imageUrl || "https://via.placeholder.com/150", // Hình ảnh mặc định nếu không có
